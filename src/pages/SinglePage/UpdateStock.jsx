...
const navigate = useNavigate()
  ...
  const onSubmit = async (data) => {
    ...
    // Loop through form data and append it correctly
    Object.entries(data).forEach(([key, value]) => {
      if (key === "image" && value instanceof File) {
        // If it's an image (file), append it correctly
    ...
    // Log the form data in a readable format (use this for debugging)
    for (let [key, value] of formData.entries()) {
      console.log(`${key}: ${value}`);
    }
  
    // Submit the formData to the mutation
  ...
  return (
    <div className="flex items-center justify-center p-4">
      <Card className="w-full max-w-5xl shadow-xl rounded-lg md:p-6 p-3 bg-white">
        <CardHeader>
...
<div className="grid gap-6 md:grid-cols-2 grid-cols-1">
      ...
      <div className="flex flex-col gap-2 col-span-1 ">
        ...
        :(
          productData?.image && (
            <img
              src={`${import.meta.env.VITE_BASE_URL}/images/${productData.image}`}
              alt={productData?.productName}
              className=" w-32 object-contain rounded-xl border"
            />
          ) 
